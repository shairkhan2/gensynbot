...
BACKUP_USERDATA_DIR = "/root/gensynbot/backup-userdata"
SYNC_BACKUP_DIR = "/root/gensynbot/sync-backup"
...
PEER_CACHE_FILE = "/root/gensynbot/peer_info.json"
...
def gensyn_soft_update(chat_id):
    ...
    backup_dir = "/root/gensynbot/soft-update-backup"
...
def gensyn_hard_update(chat_id):
    ...
    backup_dir = "/root/gensynbot/hard-update-backup"
    os.makedirs(backup_dir, exist_ok=True)
    try:
        for path in backup_paths:
            if os.path.exists(path):
...
def format_gensyn_status():
    ...
    EOA_CACHE_FILE = "/root/gensynbot/eoa_cache.json"
...
def who_handler(message):
    if message.from_user.id == USER_ID:
        bot.send_message(message.chat.id, f"👤 This is your Gensyn Bot")
    ...
    try:
        if call.data == 'check_ip':
            try:
                ...
                markup = get_menu()
                bot.edit_message_text(f"🌐 Current Public IP: {ip}", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
            except Exception as e:
                markup = get_menu()
                bot.edit_message_text(f"❌ Error checking IP: {str(e)}", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
            ...
            with login_lock:
                ...
                try:
                    ...
                    venv_python = "/root/gensynbot/.venv/bin/python3"
                    signup_script = "/root/gensynbot/signup.py"
                    venv_site_packages = "/root/gensynbot/.venv/lib/python3.12/site-packages"
            ...
            try:
                status_message = format_gensyn_status()
                markup = get_menu()
                bot.edit_message_text(status_message, chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode="HTML", reply_markup=markup)
            except Exception as e:
                logging.error(f"Error in gensyn_status callback: {str(e)}")
                markup = get_menu()
                bot.edit_message_text("❌ Error getting status. Check logs.", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
            ...
            try:
                subprocess.run("screen -S gensyn -X quit", shell=True, check=True)
                markup = get_menu()
                bot.edit_message_text("🛑 gensyn screen killed (and all child processes).", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
            except subprocess.CalledProcessError as e:
                markup = get_menu()
                bot.edit_message_text(f"❌ Failed to kill gensyn screen: {str(e)}", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
            ...
            bot.edit_message_text("What do you want to update?", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
            ...
            bot.edit_message_text("Choose update type:", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
            ...
            markup = get_menu()
            bot.edit_message_text("Log skipped.", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
            
    except Exception as e:
        logging.error(f"Error in callback_query: {str(e)}")
        try:
            markup = get_menu()
            bot.edit_message_text("❌ An error occurred. Check logs.", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
        except:
